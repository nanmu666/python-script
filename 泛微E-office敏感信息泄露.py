...
def check_url(url):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            truncated_response = '\n'.join(lines)
            if "UserSelect" in truncated_response:
...
if __name__ == '__main__':
    show = r'''
	泛微E-office存在MySQL配置信息泄露漏洞
	'''
    print(show + '\n')
    arg = ArgumentParser(description='check_url By when')
    arg.add_argument("-u",
                     "--url",
                     help="Target URL; Example:python3 .\泛微E-office敏感信息泄露.py -u http://ip:port")
    arg.add_argument("-f",
                     "--file",
                     help="Target URL; Example:python3 .\泛微E-office敏感信息泄露.py -f url.txt")
    args = arg.parse_args()
    url = args.url
    filename = args.file
    print("[+]任务开始.....")
