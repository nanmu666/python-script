...
def check_url(url):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            truncated_response = '\n'.join(lines)
            if "html" in truncated_response:
                print("\033[32m[+]{} 存在漏洞 \033[0m".format(url))
                wirte_targets(url, "！！！！！！.txt")
        else:
...
if __name__ == '__main__':
    ...
    arg.add_argument("-u",
                     "--url",
                     help="Target URL; Example:python3 .\edu前台重置通杀.py -u http://ip:port")
    arg.add_argument("-f",
                     "--file",
                     help="Target URL; Example:python3 .\edu前台重置通杀.py -f url.txt")
    args = arg.parse_args()
    url = args.url
    filename = args.file
    print("[+]任务开始.....")
